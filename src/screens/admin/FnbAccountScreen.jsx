 ...
 *
 * This is a direct clone of FrontDeskAccountScreen.jsx, retargeted at
 * role = 'fnb' instead of 'frontdesk' — same account-creation flow,
 * same Edit/Remove/Reset Password actions, same audit logging, same
 * desktop two-panel layout. See that file's own header comment for the
 * full history of WHY it's built the way it is (the secondarySupabase
 * session-isolation trick for account creation, the active=false
 * "Remove" pattern, the desktop/tablet layout reasoning, the photo-sync
 * fix) — none of that changed here, only which role gets assigned.
 *
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={[styles.staffCard, isWide && styles.staffCardWide]}>
        ...
        >
          {creatingStaff ? <ActivityIndicator color={colors.white} /> : <Text style={styles.createButtonText}>Create F&amp;B Account</Text>}
        </TouchableOpacity>
      </View>
      ...
      <View style={[styles.staffListCard, isWide && styles.staffListCardWide]}>
        <View style={styles.cardHeaderRow}>
          ...
          <Text style={styles.sectionTitle}>Existing F&amp;B Accounts</Text>
        </View>
        ...
        {filteredStaff.length === 0 ? (
          <Text style={styles.emptyText}>
            {staffAccounts.length === 0 ? 'No F&B accounts yet.' : 'No accounts match your search.'}
          </Text>
        ) : (
